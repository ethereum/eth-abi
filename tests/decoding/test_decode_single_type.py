...
def test_decode_accepts_bytes(input, expected):
    output = decode_single('int256', input.decode('hex'))
    assert output == expected
...
@pytest.mark.parametrize(
    ...
    (
        (
            ...
            'test\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00',
        ),
        (
            '0x6162636465666768696a6b6c6d6e6f707172737475767778797a000000000000',
            'abcdefghijklmnopqrstuvwxyz\x00\x00\x00\x00\x00\x00',
        ),
        (
            '0x3031323334353637383921402324255e262a2829000000000000000000000000',
            '0123456789!@#$%^&*()\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00',
        ),
        (
