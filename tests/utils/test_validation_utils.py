...
@pytest.mark.parametrize(
    'bytes_value',
    (b'', b'\x00', b'david attenborough', b'\xff' * 32, bytearray(b'\x01\x02\x03\x04\xff'))
)
...
@pytest.mark.parametrize(
    'non_bytes_value',
    ('', 123, [b'david attenborough'], {b'david attenborough'}, (b'david attenborough',))
)
def test_validate_bytes_param_raises_for_non_bytes_types(non_bytes_value):
    param_name = 'test_param'
    with pytest.raises(
        TypeError,
        match=f"The `{param_name}` value must be of bytes type. Got {type(non_bytes_value)}"
    ):
        validate_bytes_param(non_bytes_value, param_name)


@pytest.mark.parametrize('list_like_value', ([1, 2], (1, 2), ['1', 2], ('1', 2), [], tuple()))
